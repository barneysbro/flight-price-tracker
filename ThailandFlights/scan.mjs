...
async function main() {
  const [fromArg, toArg, daysArg = '8,9,10', destinationArg = 'BKK', airlineArg = 'ALL'] = process.argv.slice(2);
  const refresh = process.argv.includes('--refresh') || process.argv.includes('--force');
  const origin = 'TPE';
  const destination = (destinationArg.startsWith('--') ? 'BKK' : destinationArg).toUpperCase();
  const airline = (airlineArg.startsWith('--') ? 'ALL' : airlineArg).toUpperCase().split('+').sort().join('+');
  if (!fromArg || !toArg || !['BKK', 'DMK'].includes(destination) || airline !== 'ALL') throw new Error('用法: node scan.mjs 2026-10-01 2026-10-31 8,9,10 BKK ALL');
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (process.argv[2] === '--self-test') {
    ...
    const direct = Buffer.from(new URL(urlFor('2026-10-01', '2026-10-08', 'TPE', 'BKK', 'ALL')).searchParams.get('tfs'), 'base64url').toString('latin1');
