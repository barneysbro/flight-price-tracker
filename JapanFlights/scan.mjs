...
export function urlFor(departure, returning, origin = 'TPE', destination = 'MAD', airline = 'CZ') {
  const airlineCodes = airline === 'ALL' ? [] : (Array.isArray(airline) ? airline : airline.split('+')).sort();
  const airlineFields = airlineCodes.map(code => `2\x02${code}`).join('');
  const legLength = String.fromCharCode(34 + (airlineCodes.length - 1) * 4 + (airline === 'ALL' ? 2 : 0));
  const bytes = Buffer.from(TEMPLATE, 'base64url').toString('latin1')
    ...
    .replaceAll('2\x02CZ', airline === 'ALL' ? '(\x00' : airlineFields);
...
async function main() {
  const [fromArg, toArg, daysArg = '8,9,10', destinationArg = 'NRT', airlineArg = 'ALL'] = process.argv.slice(2);
  const refresh = process.argv.includes('--refresh') || process.argv.includes('--force');
  const origin = 'TPE';
  const destination = (destinationArg.startsWith('--') ? 'NRT' : destinationArg).toUpperCase();
  const airline = (airlineArg.startsWith('--') ? 'ALL' : airlineArg).toUpperCase().split('+').sort().join('+');
  if (!fromArg || !toArg || !['HND', 'NRT', 'KIX', 'ITM', 'FUK', 'CTS', 'NGO', 'OKA'].includes(destination) || airline !== 'ALL') throw new Error('用法: node scan.mjs 2026-10-01 2026-10-31 8,9,10 NRT ALL');
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (process.argv[2] === '--self-test') {
    ...
    const direct = Buffer.from(new URL(urlFor('2026-10-01', '2026-10-08', 'TPE', 'NRT', 'ALL')).searchParams.get('tfs'), 'base64url').toString('latin1');
    assert.equal(direct.match(/\x28\x00/g).length, 2);
    assert.doesNotMatch(direct, /CZ/);
